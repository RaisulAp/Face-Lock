...
import { Users, CheckCircle2, ClockAlert, XCircle, Activity, ArrowRight, Sparkles } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-gray-900 tracking-tight">Ringkasan Operasional Kehadiran</h1>
                  ...
                  {isRecentLoading ? (
                    ...
                    recentAttendances.map((item) => (
                      <tr key={item.id} className="hover:bg-gray-50/50">
                        ...
                        <td className="px-4 py-2">
                          <p className="font-semibold text-gray-900">{item.employee_name || "Karyawan"}</p>
                          <span className="text-[11px] text-gray-400">{item.employee_id.slice(0, 8)}...</span>
                        </td>
          ...
          <Card className="bg-gradient-to-br from-indigo-900 to-slate-900 text-white border-0 shadow-md">
            ...
            <CardContent className="space-y-3">
              <p className="text-xs text-indigo-200 leading-relaxed">
                Terdapat {summary?.pending_count ?? 0} permohonan kehadiran dengan anomali geofence atau
                kemiripan wajah sedang menunggu tinjauan supervisor.
              </p>
