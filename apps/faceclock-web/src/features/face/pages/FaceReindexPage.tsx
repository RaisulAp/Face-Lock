...
export function FaceReindexPage() {
    ...
    const { data: pastJobs, refetch: refetchJobs } = useQuery<FaceReindexJob[]>({
        queryKey: ["face", "reindex-jobs"],
        queryFn: () => api.get<FaceReindexJob[]>("/api/v1/face/reindex-jobs"),
    });
    ...
    const isRunning =
        activeJobDetail?.status === "pending" || activeJobDetail?.status === "running";

    const progressPercent = activeJobDetail?.total_count
        ? Math.round(
            ((activeJobDetail.processed_count || 0) / activeJobDetail.total_count) * 100
        )
        : 0;
            ...
            <div className="p-4 bg-amber-50 border border-amber-300 rounded-xl flex items-start gap-3 text-amber-900 text-xs">
                ...
                <div className="space-y-1">
                    <strong className="font-semibold block">
                        Aturan Keamanan Sistem & Non-Blocking Reindex:
                    </strong>
                    <ul className="list-disc list-inside space-y-0.5 text-amber-800">
                        <li>
                            <strong>Dry-Run Wajib:</strong> Anda harus menjalankan simulasi (dry-run) terlebih
                            dahulu untuk melihat dampak sebelum eksekusi aktual diaktifkan.
                        </li>
                        <li>
                            <strong>Selama proses berjalan:</strong> Karyawan tidak dapat melakukan pendaftaran
                            wajah baru (HTTP 409 REINDEX_IN_PROGRESS).
                        </li>
                        <li>
                    ...
                    {dryRunResult && (
                        <div className="mt-4 p-3.5 bg-indigo-50 border border-indigo-200 rounded-xl text-xs space-y-1">
                            <span className="font-bold text-indigo-900 block">
                                Hasil Analisis Dry-Run Terverifikasi:
                            </span>
                            <p className="text-indigo-800">
                                Model Sumber: <strong>{dryRunResult.from_model_version || "Aktif"}</strong> ➔
                                Target: <strong>{dryRunResult.to_model_version}</strong>
                            </p>
                            <p className="text-indigo-800">
                                Total Foto Ditemukan: <strong>{dryRunResult.total_count}</strong> foto referensi
                                pada <strong>{dryRunResult.affected_employee_count}</strong> karyawan.
                            </p>
                ...
                <Card className="border-indigo-300 shadow-sm">
                    <CardHeader>
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                <RefreshCw
                                    className={`w-4 h-4 text-indigo-600 ${isRunning ? "animate-spin" : ""}`}
                                />
                    ...
                    <CardContent className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
                            ...
                            <div className="p-2.5 bg-gray-50 rounded-lg border border-gray-100">
                                ...
                                <span className="text-rose-700 font-bold text-base">
                                    {activeJobDetail.failed_count ?? 0}
                                </span>
                            </div>
                        ...
                        {activeJobDetail.incomplete_employees &&
                            activeJobDetail.incomplete_employees.length > 0 && (
                                <div className="p-3 bg-rose-50 border border-rose-200 rounded-xl space-y-2 text-xs">
                                    <span className="font-bold text-rose-900 block">
                                        Karyawan Yang Perlu Pendaftaran Ulang:
                                    </span>
                ...
                <CardContent>
                    {!pastJobs || pastJobs.length === 0 ? (
                        <p className="text-center py-6 text-xs text-gray-400">
                            Belum ada riwayat pekerjaan reindex.
                        </p>
                    ) : (
