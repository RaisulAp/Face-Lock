...
export function UsersPage() {
  ...
  const openRolesModal = (u: User) => {
    ...
    const currentRoleIds = (rolesList || []).filter((r) => u.roles?.includes(r.name)).map((r) => r.id);
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-xs">Daftar Akun Pengguna ({usersRes?.meta?.total || 0})</CardTitle>
        </CardHeader>
        <CardContent className="p-0">
                    ...
                    <tr key={u.id} className="hover:bg-gray-50/50">
                      <td className="px-4 py-3">
                        ...
                        {u.must_change_password && (
                          <span className="text-[10px] text-amber-600 font-medium">Wajib ganti sandi</span>
                        )}
                      </td>
          ...
          {usersRes?.meta && usersRes.meta.total_pages > 1 && (
            <div className="p-4 border-t border-gray-100">
              <Pagination page={page} totalPages={usersRes.meta.total_pages} onPageChange={setPage} />
            </div>
          )}
        </CardContent>
      </Card>
      ...
      {/* Modal: Create User */}
      <Modal open={createOpen} onClose={() => setCreateOpen(false)} title="Tambah Akun Pengguna Baru">
        <form onSubmit={handleCreateUser} className="space-y-4">
          <div>
            <label className="block text-xs font-semibold text-gray-700 mb-1">Alamat Email (Login) *</label>
            <Input
              type="email"
              required
              value={createEmail}
          ...
          <div>
            ...
            <p className="text-[10px] text-gray-400 mt-1">
              Jika dikosongkan, sistem akan meng-generate password awal dan user wajib menggantinya saat
              login.
            </p>
          </div>

          <div>
            <label className="block text-xs font-semibold text-gray-700 mb-1">Tetapkan Peran (Roles)</label>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      if (e.target.checked) {
                        ...
                        setCreateSelectedRoleIds(createSelectedRoleIds.filter((id) => id !== r.id));
                      }
                    }}
      ...
      >
        <div className="space-y-4">
          <p className="text-xs text-gray-500">
            Pilih peran yang ingin diberikan kepada pengguna ini. Seluruh daftar peran lama akan digantikan
            sesuai pilihan di bawah (Replace).
          </p>
          ...
          <div className="flex justify-end gap-2 pt-2 border-t border-gray-100">
            <Button type="button" variant="outline" size="sm" onClick={() => setRolesOpen(false)}>
              Batal
            </Button>
            <Button
              type="button"
      ...
      >
        <form onSubmit={handleResetPassword} className="space-y-4">
          ...
          <div className="flex justify-end gap-2 pt-2 border-t border-gray-100">
            <Button type="button" variant="outline" size="sm" onClick={() => setResetPwdOpen(false)}>
              Batal
            </Button>
            <Button type="submit" variant="primary" size="sm" isLoading={isSubmitting}>
              Simpan Kata Sandi
