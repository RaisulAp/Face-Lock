...
export const employeeApi = {
    ...
    getContext: () => api.get<AttendanceContextData>("/attendances/context"),
    ...
    submitClock: async (
        ...
        const endpoint = action === "check_in" ? "/attendances/clock-in" : "/attendances/clock-out";
    ...
    getMyHistory: (params: AttendanceHistoryParams = {}) => {
        ...
        return api.get<PaginatedEnvelope<EmployeeAttendanceItem>>(`/attendances/me${qs ? `?${qs}` : ""}`);
    },

    getAttendanceDetail: (id: string) => api.get<EmployeeAttendanceItem>(`/attendances/${id}`),

    // Biometric Consent (UU PDP No. 27/2022)
    getConsentDocument: () => api.get<ConsentDocumentData>("/consents/document"),

    getMyConsent: () => api.get<ConsentMeData>("/consents/me"),

    grantConsent: (documentVersion: string) =>
        api.post<{ status: string; document_version: string; granted_at: string }>("/consents", {
            document_version: documentVersion,
            agreed: true,
        }),

    withdrawConsent: (reason: string) => api.post<WithdrawConsentResponse>("/consents/withdraw", { reason }),

    // Face Enrollment
    getMyEnrollmentStatus: () => api.get<EnrollmentStatusMeData>("/face/enrollment-status/me"),

    createEnrollmentSession: (mode: "replace" | "append" = "replace") =>
        api.post<EnrollmentSessionData>("/face/enrollments", { mode }),

    getEnrollmentSession: (sessionId: string) =>
        api.get<EnrollmentSessionData>(`/face/enrollments/${sessionId}`),
    ...
    uploadEnrollmentPhoto: (sessionId: string, photo: Blob, captureSource = "web_camera") => {
        ...
        return api.postForm<UploadEnrollmentPhotoResult>(`/face/enrollments/${sessionId}/photos`, formData);
    },

    deleteEnrollmentPhoto: (sessionId: string, photoId: string) =>
        api.delete<void>(`/face/enrollments/${sessionId}/photos/${photoId}`),

    commitEnrollmentSession: (sessionId: string, forceDuplicate = false, reason?: string) =>
        api.post<CommitSessionResult>(`/face/enrollments/${sessionId}/commit`, {
            force_duplicate: forceDuplicate,
            reason,
        }),

    cancelEnrollmentSession: (sessionId: string) => api.delete<void>(`/face/enrollments/${sessionId}`),

    // Profile & Security
    getMyProfile: () => api.get<EmployeeProfileData>("/employees/me"),
